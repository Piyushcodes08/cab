...
const allQuestions = [
  { q: '🗼 Which city is home to the Eiffel Tower?', options: ['Rome', 'Paris', 'Berlin', 'Madrid'], answer: 1, diff: 'easy' },
  { q: '🏝️ Which country has the most islands?', options: ['Indonesia', 'Norway', 'Sweden', 'Philippines'], answer: 2, diff: 'medium' },
  { q: '🌊 What is the world\'s largest ocean?', options: ['Atlantic', 'Indian', 'Arctic', 'Pacific'], answer: 3, diff: 'easy' },
  { q: '🗽 Where is the Statue of Liberty?', options: ['Washington', 'Boston', 'New York', 'Chicago'], answer: 2, diff: 'easy' },
  { q: '🌋 Most active volcanoes are in...?', options: ['Japan', 'Indonesia', 'Iceland', 'USA'], answer: 1, diff: 'hard' },
  { q: '🏔️ What is the tallest mountain?', options: ['K2', 'Kangchenjunga', 'Everest', 'Makalu'], answer: 2, diff: 'easy' },
  { q: '🏜️ Largest desert on Earth?', options: ['Sahara', 'Antarctic', 'Gobi', 'Arabian'], answer: 1, diff: 'hard' },
  { q: '🇯🇵 What is the capital of Japan?', options: ['Osaka', 'Tokyo', 'Kyoto', 'Hiroshima'], answer: 1, diff: 'easy' },
];
...
const AboutGame = () => {
  ...
  const startGame = () => {
    const picked = pickRandom(allQuestions, ROUNDS);
    setQuestions(picked);
    setCurrent(0); setScore(0); setXp(0); setSelected(null);
    setTimeLeft(TIMER); setStreak(0); setMaxStreak(0); setAnswers([]);
    setScreen('play');
  };

  const getResultData = () => {
    const pct = (score / ROUNDS) * 100;
    if (pct === 100) return { emoji: '🏆', title: 'LEGENDARY!', sub: 'Absolute travel genius.', tier: 'legendary', color: '#fbbf24' };
    if (pct >= 80)  return { emoji: '💎', title: 'AMAZING!', sub: 'You really know your stuff.', tier: 'diamond', color: '#818cf8' };
    if (pct >= 60)  return { emoji: '🌍', title: 'GREAT JOB!', sub: 'Solid knowledge!', tier: 'gold', color: '#4ade80' };
    if (pct >= 40)  return { emoji: '✈️', title: 'NOT BAD!', sub: 'Room to explore more.', tier: 'silver', color: '#94a3b8' };
    return { emoji: '🗺️', title: 'KEEP EXPLORING!', sub: 'Travel and learn!', tier: 'bronze', color: '#fb923c' };
  };
  ...
  return (
    <div className="ag-wrap">
      ...
      <div className="ag-particles" aria-hidden="true">
        {[...Array(6)].map((_, i) => <span key={i} className={`ag-particle ag-p${i + 1}`} />)}
      </div>
      ...
      {screen === 'start' && (
        <div className="ag-card ag-start">
          ...
          <p className="ag-hero-sub">Test your world knowledge in {ROUNDS} quick rounds!</p>
        ...
        <div className={`ag-card ag-play ${shakeCard ? 'ag-shake' : ''}`}>
          ...
          {showConfetti && (
            <div className="ag-confetti" aria-hidden="true">
              {[...Array(18)].map((_, i) => <span key={i} className="ag-conf-piece" style={{ '--i': i }} />)}
            </div>
          )}
          ...
          <div className="ag-play-top">
            ...
            {q.diff && (
              <span className={`ag-diff ag-diff-${q.diff}`}>
                {q.diff === 'easy' ? '🟢' : q.diff === 'medium' ? '🟡' : '🔴'} {q.diff}
              </span>
            )}
          ...
          <div className="ag-xp-row">
            ...
            {streak >= 2 && <span className="ag-xp-bonus">+{XP_STREAK * streak} bonus!</span>}
          </div>
            ...
            {q.options?.map((opt, i) => {
              ...
              return (
                <button key={i} className={cls} onClick={() => handleAnswer(i)} disabled={selected !== null}>
                  ...
                  {selected !== null && i === q.answer && <span className="ag-opt-check">✓</span>}
                  {selected === i && i !== q.answer && <span className="ag-opt-x">✗</span>}
                </button>
              );
            })}
          </div>
      ...
      {screen === 'result' && (() => {
        ...
        return (
          <div className="ag-card ag-result">
            <div className="ag-result-glow" style={{ '--glow-color': r.color }} />

            <div className="ag-result-emoji">{r.emoji}</div>

            <h3 className="ag-result-title" style={{ color: r.color }}>{r.title}</h3>
            ...
            <div className="ag-score-ring">
              <svg viewBox="0 0 120 120" className="ag-score-svg">
                ...
                <circle cx="60" cy="60" r="52"
                  fill="none" stroke={r.color} strokeWidth="8"
            ...
            <div className="ag-result-stats">
              <div className="ag-rstat">
                <span className="ag-rstat-val" style={{ color: r.color }}>⚡ {xp}</span>
                <span className="ag-rstat-label">Total XP</span>
              </div>
              <div className="ag-rstat">
                <span className="ag-rstat-val" style={{ color: r.color }}>🔥 {maxStreak}</span>
                <span className="ag-rstat-label">Best Streak</span>
              </div>
              <div className="ag-rstat">
                <span className="ag-rstat-val" style={{ color: r.color }}>{pct}%</span>
            ...
            <div className="ag-answer-dots">
              {answers.map((a, i) => (
                <span key={i} className={`ag-dot ${a.correct ? 'ag-dot-ok' : 'ag-dot-fail'}`}>
                  {a.correct ? '✓' : '✗'}
                </span>
              ))}
            </div>
